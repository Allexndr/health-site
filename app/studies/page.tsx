import Link from 'next/link'
import { 
  BeakerIcon,
  PlusIcon,
  MagnifyingGlassIcon,
  FunnelIcon,
  EyeIcon,
  PencilIcon,
  TrashIcon,
  DocumentArrowDownIcon,
  ShareIcon,
  CalendarIcon,
  HeartIcon,
  BellIcon,
  ClipboardDocumentListIcon,
  CameraIcon,
  ChartBarIcon,
  ClockIcon
} from '@heroicons/react/24/outline'

const studies = [
  { 
    id: 1, 
    patient: 'Анна Петрова', 
    type: 'Общий анализ крови',
    date: '2024-01-15',
    status: 'Завершено',
    doctor: 'Др. Смирнов А.В.',
    result: 'Норма',
    priority: 'Обычный',
    image: '🩸',
    description: 'Контрольный анализ на фоне терапии гипертонии',
    values: {
      'Hb': '135 г/л (норма 120-160)',
      'Эритроциты': '4.2×10¹²/л (норма 3.7-5.1)',
      'Лейкоциты': '6.8×10⁹/л (норма 4.0-9.0)',
      'Тромбоциты': '285×10⁹/л (норма 150-400)',
      'СОЭ': '12 мм/ч (норма до 20)'
    }
  },
  { 
    id: 2, 
    patient: 'Михаил Иванов', 
    type: 'Биохимический анализ крови',
    date: '2024-01-14',
    status: 'Завершено',
    doctor: 'Др. Козлова Е.И.',
    result: 'Требует внимания',
    priority: 'Высокий',
    image: '🧪',
    description: 'Контроль гликемии и функции почек при СД 2 типа',
    values: {
      'Глюкоза': '12.4 ммоль/л (норма 3.3-5.5)',
      'HbA1c': '8.2% (норма <6.5%)',
      'Креатинин': '145 мкмоль/л (норма 62-106)',
      'Мочевина': '8.9 ммоль/л (норма 2.5-8.3)',
      'Общий холестерин': '6.8 ммоль/л (норма <5.2)'
    }
  },
  { 
    id: 3, 
    patient: 'Елена Сидорова', 
    type: 'Спирометрия',
    date: '2024-01-13',
    status: 'Завершено',
    doctor: 'Др. Петров С.А.',
    result: 'Норма',
    priority: 'Обычный',
    image: '🫁',
    description: 'Контроль функции внешнего дыхания при астме',
    values: {
      'ФЖЕЛ': '3.2 л (норма 3.0-4.2)',
      'ОФВ1': '2.8 л (норма 2.5-3.5)',
      'ОФВ1/ФЖЕЛ': '87% (норма >70%)',
      'ПСВ': '450 л/мин (норма 400-600)',
      'МОС25-75': '3.1 л/с (норма 2.5-4.0)'
    }
  },
  { 
    id: 4, 
    patient: 'Дмитрий Козлов', 
    type: 'ЭКГ с нагрузочной пробой',
    date: '2024-01-12',
    status: 'Завершено',
    doctor: 'Др. Смирнов А.В.',
    result: 'Требует внимания',
    priority: 'Высокий',
    image: '❤️',
    description: 'Диагностика ИБС, стенокардии напряжения',
    values: {
      'ЧСС покой': '72 уд/мин (норма 60-100)',
      'ЧСС нагрузка': '145 уд/мин',
      'АД покой': '140/90 мм рт.ст.',
      'АД нагрузка': '180/100 мм рт.ст.',
      'Заключение': 'Положительная нагрузочная проба, признаки ишемии'
    }
  },
  { 
    id: 5, 
    patient: 'Ольга Морозова', 
    type: 'МРТ шейного отдела позвоночника',
    date: '2024-01-11',
    status: 'Завершено',
    doctor: 'Др. Козлова Е.И.',
    result: 'Патология',
    priority: 'Средний',
    image: '🦴',
    description: 'Диагностика остеохондроза, протрузий дисков',
    values: {
      'С3-С4': 'Протрузия 2.5 мм',
      'С4-С5': 'Протрузия 3.1 мм',
      'С5-С6': 'Протрузия 2.8 мм',
      'С6-С7': 'Протрузия 1.9 мм',
      'Заключение': 'Множественные протрузии, признаки остеохондроза'
    }
  },
  { 
    id: 6, 
    patient: 'Александр Смирнов', 
    type: 'ФГДС с биопсией',
    date: '2024-01-10',
    status: 'Завершено',
    doctor: 'Др. Петров С.А.',
    result: 'Норма',
    priority: 'Обычный',
    image: '🔍',
    description: 'Контроль гастрита, исключение язвенной болезни',
    values: {
      'Пищевод': 'Норма',
      'Кардия': 'Норма',
      'Желудок': 'Поверхностный гастрит',
      'ДПК': 'Норма',
      'Биопсия': 'Воспалительные изменения, H.pylori (-)'
    }
  },
  { 
    id: 7, 
    patient: 'Мария Волкова', 
    type: 'Анализ на железо',
    date: '2024-01-09',
    status: 'Завершено',
    doctor: 'Др. Козлова Е.И.',
    result: 'Патология',
    priority: 'Высокий',
    image: '🩸',
    description: 'Диагностика железодефицитной анемии',
    values: {
      'Hb': '95 г/л (норма 120-160)',
      'Ферритин': '8 нг/мл (норма 15-200)',
      'Сывороточное железо': '4.2 мкмоль/л (норма 10.7-32.2)',
      'ОЖСС': '85 мкмоль/л (норма 45-75)',
      'Трансферрин': '3.8 г/л (норма 2.0-4.0)'
    }
  },
  { 
    id: 8, 
    patient: 'Владимир Новиков', 
    type: 'КТ органов грудной клетки',
    date: '2024-01-08',
    status: 'Завершено',
    doctor: 'Др. Петров С.А.',
    result: 'Патология',
    priority: 'Высокий',
    image: '🫁',
    description: 'Диагностика ХОБЛ, эмфиземы легких',
    values: {
      'Эмфизема': 'Диффузная, средней степени',
      'Бронхоэктазы': 'Множественные, двусторонние',
      'Инфильтраты': 'Нет',
      'Плевра': 'Спайки справа',
      'Заключение': 'ХОБЛ, эмфизема, бронхоэктатическая болезнь'
    }
  },
  { 
    id: 9, 
    patient: 'Татьяна Лебедева', 
    type: 'Анализ гормонов щитовидной железы',
    date: '2024-01-07',
    status: 'Завершено',
    doctor: 'Др. Смирнов А.В.',
    result: 'Норма',
    priority: 'Обычный',
    image: '🦋',
    description: 'Контроль функции щитовидной железы при гипотиреозе',
    values: {
      'ТТГ': '2.1 мЕд/л (норма 0.4-4.0)',
      'Т4 свободный': '18.5 пмоль/л (норма 9-25)',
      'Т3 свободный': '4.2 пмоль/л (норма 2.6-5.7)',
      'АТ-ТПО': '45 МЕ/мл (норма <60)',
      'АТ-ТГ': '12 МЕ/мл (норма <60)'
    }
  },
  { 
    id: 10, 
    patient: 'Сергей Медведев', 
    type: 'УЗИ почек и мочевого пузыря',
    date: '2024-01-06',
    status: 'Завершено',
    doctor: 'Др. Козлова Е.И.',
    result: 'Патология',
    priority: 'Средний',
    image: '🫘',
    description: 'Диагностика мочекаменной болезни',
    values: {
      'Правая почка': 'Норма, размеры 105×52×48 мм',
      'Левая почка': 'Конкремент 6×4 мм в средней чашке',
      'Мочевой пузырь': 'Норма, стенки 3 мм',
      'Мочеточники': 'Не расширены',
      'Заключение': 'МКБ, камень левой почки 6 мм'
    }
  }
]

const navItems = [
  { name: 'Дашборд', href: '/dashboard', icon: '📊', color: 'hover:bg-blue-50 hover:text-blue-600' },
  { name: 'Пациенты', href: '/patients', icon: '👥', color: 'hover:bg-green-50 hover:text-green-600' },
  { name: 'Исследования', href: '/studies', icon: '🔬', color: 'bg-purple-50 text-purple-600' },
  { name: 'Записи', href: '/appointments', icon: '📅', color: 'hover:bg-orange-50 hover:text-orange-600' },
  { name: 'Отчеты', href: '/reports', icon: '📋', color: 'hover:bg-red-50 hover:text-red-600' },
  { name: 'Аналитика', href: '/analytics', icon: '📈', color: 'hover:bg-indigo-50 hover:text-indigo-600' },
  { name: 'Настройки', href: '/settings', icon: '⚙️', color: 'hover:bg-gray-50 hover:text-gray-600' },
]

export default function StudiesPage() {
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 dark:from-gray-900 dark:via-slate-800 dark:to-indigo-900">
      {/* Animated Background Elements */}
      <div className="absolute inset-0 overflow-hidden">
        <div className="absolute -top-40 -right-40 w-80 h-80 bg-blue-400/20 rounded-full blur-3xl animate-pulse"></div>
        <div className="absolute -bottom-40 -left-40 w-80 h-80 bg-indigo-400/20 rounded-full blur-3xl animate-pulse animation-delay-2000"></div>
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-cyan-400/10 rounded-full blur-3xl animate-pulse animation-delay-4000"></div>
      </div>

      {/* Header */}
      <header className="relative backdrop-blur-sm bg-white/95 dark:bg-gray-900/95 border-b border-gray-200/50 dark:border-gray-700/50 shadow-xl">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <div className="flex h-20 items-center justify-between">
            <div className="flex items-center space-x-4">
              <div className="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-xl p-3 shadow-lg">
                <HeartIcon className="h-8 w-8 text-white" />
              </div>
              <div>
                <span className="text-2xl font-bold bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">MediCloud</span>
                <p className="text-xs text-gray-500 -mt-1 font-medium">Medical Solutions Platform</p>
              </div>
            </div>
            
            {/* Navigation Menu */}
            <nav className="hidden lg:flex items-center space-x-1">
              {navItems.map((item) => (
                <Link
                  key={item.name}
                  href={item.href}
                  className={`px-4 py-2 text-gray-700 dark:text-gray-300 ${item.color} rounded-lg transition-all duration-200 font-medium flex items-center space-x-2 group`}
                >
                  <span className="text-lg group-hover:scale-110 transition-transform duration-200">{item.icon}</span>
                  <span>{item.name}</span>
                </Link>
              ))}
            </nav>

            <div className="flex items-center space-x-4">
              <div className="hidden sm:flex items-center space-x-2 px-4 py-2 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-800">
                <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div>
                <span className="text-sm text-green-700 dark:text-green-400 font-medium">Система онлайн</span>
              </div>
              <div className="flex items-center space-x-2">
                <button className="p-2 text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 transition-colors">
                  <BellIcon className="h-5 w-5" />
                </button>
                <button className="p-2 text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 transition-colors">
                  <MagnifyingGlassIcon className="h-5 w-5" />
                </button>
              </div>
              <div className="flex items-center space-x-2">
                <div className="w-8 h-8 bg-gradient-to-r from-blue-500 to-indigo-500 rounded-full flex items-center justify-center">
                  <span className="text-white text-sm font-bold">АС</span>
                </div>
                <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Александр Смирнов</span>
              </div>
            </div>
          </div>
        </div>
      </header>

      {/* Main Content */}
      <main className="relative z-10">
        <div className="mx-auto max-w-7xl px-6 py-8 lg:px-8">
          {/* Page Header */}
          <div className="flex items-center justify-between mb-8">
            <div>
              <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">Медицинские исследования</h1>
              <p className="text-gray-600 dark:text-gray-300">Управление результатами диагностики и анализов</p>
            </div>
            <Link 
              href="/studies/new"
              className="bg-gradient-to-r from-purple-600 to-pink-600 px-6 py-3 text-white font-semibold rounded-xl shadow-lg hover:shadow-xl transition-all duration-200 transform hover:scale-105 flex items-center space-x-2"
            >
              <PlusIcon className="h-5 w-5" />
              <span>Новое исследование</span>
            </Link>
          </div>

          {/* Stats Cards */}
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-gray-600 dark:text-gray-300">Всего исследований</p>
                  <p className="text-3xl font-bold text-gray-900 dark:text-white">1,234</p>
                </div>
                <div className="bg-gradient-to-r from-blue-500 to-indigo-600 rounded-xl p-3">
                  <BeakerIcon className="h-6 w-6 text-white" />
                </div>
              </div>
            </div>
            <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-gray-600 dark:text-gray-300">Завершено</p>
                  <p className="text-3xl font-bold text-green-600">1,156</p>
                </div>
                <div className="bg-gradient-to-r from-green-500 to-emerald-600 rounded-xl p-3">
                  <ClipboardDocumentListIcon className="h-6 w-6 text-white" />
                </div>
              </div>
            </div>
            <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-gray-600 dark:text-gray-300">В процессе</p>
                  <p className="text-3xl font-bold text-yellow-600">78</p>
                </div>
                <div className="bg-gradient-to-r from-yellow-500 to-orange-600 rounded-xl p-3">
                  <ClockIcon className="h-6 w-6 text-white" />
                </div>
              </div>
            </div>
            <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-gray-600 dark:text-gray-300">Требуют внимания</p>
                  <p className="text-3xl font-bold text-red-600">23</p>
                </div>
                <div className="bg-gradient-to-r from-red-500 to-pink-600 rounded-xl p-3">
                  <HeartIcon className="h-6 w-6 text-white" />
                </div>
              </div>
            </div>
          </div>

          {/* Search and Filters */}
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg mb-8">
            <div className="flex flex-col sm:flex-row gap-4">
              <div className="flex-1">
                <div className="relative">
                  <MagnifyingGlassIcon className="absolute left-3 top-1/2 transform -translate-y-1/2 h-5 w-5 text-gray-400" />
                  <input
                    type="text"
                    placeholder="Поиск исследований по пациенту, типу или врачу..."
                    className="w-full pl-10 pr-4 py-3 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent dark:bg-gray-700 dark:text-white"
                  />
                </div>
              </div>
              <div className="flex gap-2">
                <button className="px-4 py-3 border border-gray-200 dark:border-gray-700 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors flex items-center space-x-2">
                  <FunnelIcon className="h-5 w-5" />
                  <span>Фильтры</span>
                </button>
                <button className="px-4 py-3 bg-gray-100 dark:bg-gray-700 rounded-xl hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors">
                  <ChartBarIcon className="h-5 w-5" />
                </button>
              </div>
            </div>
          </div>

          {/* Studies Grid */}
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
            {studies.map((study) => (
              <div key={study.id} className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center space-x-3">
                    <div className="w-12 h-12 bg-gradient-to-r from-purple-500 to-pink-500 rounded-full flex items-center justify-center text-white text-xl">
                      {study.image}
                    </div>
                    <div>
                      <h3 className="font-bold text-gray-900 dark:text-white">{study.patient}</h3>
                      <p className="text-sm text-gray-600 dark:text-gray-300">{study.type}</p>
                    </div>
                  </div>
                  <span className={`px-2 py-1 text-xs font-medium rounded-full ${
                    study.status === 'Завершено' ? 'bg-green-100 text-green-800' :
                    study.status === 'В процессе' ? 'bg-yellow-100 text-yellow-800' :
                    'bg-gray-100 text-gray-800'
                  }`}>
                    {study.status}
                  </span>
                </div>

                <div className="space-y-2 mb-4">
                  <div className="flex items-center space-x-2 text-sm text-gray-600 dark:text-gray-300">
                    <CalendarIcon className="h-4 w-4" />
                    <span>{study.date}</span>
                  </div>
                  <div className="flex items-center space-x-2 text-sm text-gray-600 dark:text-gray-300">
                    <span className="font-medium">Врач:</span>
                    <span>{study.doctor}</span>
                  </div>
                  <div className="flex items-center space-x-2 text-sm text-gray-600 dark:text-gray-300">
                    <span className="font-medium">Приоритет:</span>
                    <span className={`px-2 py-1 text-xs rounded-full ${
                      study.priority === 'Высокий' ? 'bg-red-100 text-red-800' :
                      study.priority === 'Средний' ? 'bg-yellow-100 text-yellow-800' :
                      'bg-gray-100 text-gray-800'
                    }`}>
                      {study.priority}
                    </span>
                  </div>
                </div>

                <div className="mb-4">
                  <p className="text-sm font-medium text-gray-900 dark:text-white mb-1">Результат:</p>
                  <p className={`text-sm ${
                    study.result === 'Норма' ? 'text-green-600' :
                    study.result === 'Требует внимания' ? 'text-yellow-600' :
                    study.result === 'Патология' ? 'text-red-600' :
                    'text-gray-600'
                  }`}>
                    {study.result}
                  </p>
                </div>

                <div className="mb-4">
                  <p className="text-sm font-medium text-gray-900 dark:text-white mb-1">Описание:</p>
                  <p className="text-sm text-gray-600 dark:text-gray-300">{study.description}</p>
                </div>

                {study.values && (
                  <div className="mb-4">
                    <p className="text-sm font-medium text-gray-900 dark:text-white mb-2">Показатели:</p>
                    <div className="space-y-1">
                      {Object.entries(study.values).map(([key, value]) => (
                        <div key={key} className="flex justify-between text-xs">
                          <span className="text-gray-600 dark:text-gray-400">{key}:</span>
                          <span className="text-gray-900 dark:text-white font-medium">{value}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                )}

                <div className="flex items-center justify-between pt-4 border-t border-gray-200 dark:border-gray-700">
                  <div className="flex space-x-2">
                    <button className="p-2 text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors">
                      <EyeIcon className="h-4 w-4" />
                    </button>
                    <button className="p-2 text-gray-400 hover:text-green-600 dark:hover:text-green-400 transition-colors">
                      <DocumentArrowDownIcon className="h-4 w-4" />
                    </button>
                    <button className="p-2 text-gray-400 hover:text-purple-600 dark:hover:text-purple-400 transition-colors">
                      <ShareIcon className="h-4 w-4" />
                    </button>
                    <button className="p-2 text-gray-400 hover:text-red-600 dark:hover:text-red-400 transition-colors">
                      <TrashIcon className="h-4 w-4" />
                    </button>
                  </div>
                  <Link 
                    href={`/studies/${study.id}`}
                    className="text-purple-600 hover:text-purple-700 dark:text-purple-400 dark:hover:text-purple-300 font-medium text-sm"
                  >
                    Подробнее →
                  </Link>
                </div>
              </div>
            ))}
          </div>

          {/* Pagination */}
          <div className="mt-8 flex items-center justify-between">
            <p className="text-sm text-gray-700 dark:text-gray-300">
              Показано 6 из 1,234 исследований
            </p>
            <div className="flex space-x-2">
              <button className="px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                Предыдущая
              </button>
              <button className="px-3 py-2 bg-purple-600 text-white rounded-lg">1</button>
              <button className="px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">2</button>
              <button className="px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">3</button>
              <button className="px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                Следующая
              </button>
            </div>
          </div>
        </div>
      </main>
    </div>
  )
}
